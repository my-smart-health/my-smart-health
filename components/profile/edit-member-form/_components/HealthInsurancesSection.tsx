...
export function HealthInsurancesSection({
  ...
  const t = useTranslations('EditMemberForm.healthInsurances');
  const handleAdd = () => {
    setHealthInsurances([
      ...healthInsurances,
            ...
            healthInsurances.map((insurance, index) => (
              <div key={index} className="p-4 border border-gray-300 rounded-lg space-y-3">
                <div className="flex justify-between items-center">
                  <h4 className="font-semibold text-sm text-gray-600">{t('insuranceItem', { index: index + 1 })}</h4>
                  <button
                    type="button"
                ...
                <input
                  ...
                  placeholder={t('insuranceNamePlaceholder')}
                  className="p-2 rounded border border-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary w-full"
                />
                <input
                  type="text"

...
async function getSmartHealthUsers(): Promise<UserProfileSH[]> {
  const cats = await prisma.category.findMany({ where: { type: PROFILE_TYPE_NOTFALLE }, select: { id: true }, orderBy: { name: 'asc' } });
...
export default async function SmartHealthPage() {
  ...
  const allCats = await prisma.category.findMany({ where: { type: PROFILE_TYPE_NOTFALLE }, select: { id: true, name: true, parentId: true } });
  ...
  return (
    <div className="grid grid-cols-1 gap-2">
      <CategoryAccordion node={tree} type={PROFILE_TYPE_NOTFALLE} isAdmin={session?.user?.role === "ADMIN"} pathToCategoryId={pathToCategoryId} />
    </div>
  );
}
